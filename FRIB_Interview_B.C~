...
void FRIB_Interview_A(){
  ...
  if(infile.is_open()){
	...
	if(N<=45 && Z<39){//only interested in certain nuclides here
	  ...
	  if(var=='s'){//stable nucleus, color it black
	    ...
	    pt->SetFillColorAlpha(kBlack,0.3);
	    TText *t1 = pt->AddText(symbol);
	    t1->SetTextColor(ci);
	    t1->SetTextAlign(22);
	    t1->SetTextSize(0.006);
	  }
	  ...
	  if(var=='n'){//neutron rich nucleus, make a color
	    pt->SetFillColorAlpha(kBlue,0.2);
	  ...
	  if(var=='u'){//unbound nucleus, make a color
	    ...
	    pt->SetFillColorAlpha(kOrange,0.2);
	    TText *t1 = pt->AddText(symbol);
	    t1->SetTextAlign(22);
  ...
  if(gROOT->IsBatch())
    c1->Print("chart_FRIB_Interview_A.png");
