...
void DNP2019_Cy(){
  ...
  TH2F *h = new TH2F("h","",50,90,140,50,50,100);
  ...
  if(infile.is_open()){
	...
	//
	if(N>=98 && N<=130 && Z<=84 && Z>=78){//only interested in certain nuclides here
	  //
	  //
	  //
	  ...
	  if(var=='p')//proton rich nucleus, make a color
	    ...
	    b->SetFillColor(TColor::GetColor(250,239,227));
 ...
 // line14->Draw("same");
  ...
  if(gROOT->IsBatch())
    c1->Print("chart_of_nuclidesCy.png");
