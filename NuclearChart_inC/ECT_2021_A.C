...
void ECT_2021_A(){
  ...
  if(infile.is_open()){
	...
	if(N<=45 && Z<39){//only interested in certain nuclides here
	  ...
	  //make Nuclei with white square in the middle (unbound)
	  ...
	  p->SetFillColor(0);
	  p->Draw("F,same");
	  Float_t x1[] = {12.3,12.3,12.7,12.7};
	  Float_t y1[] = {4.3,4.7,4.7,4.3};
	  TGraph *p1 = new TGraph(4,x1,y1);
	  p1->SetLineColor(0);
	  p1->SetFillColor(0);
	  p1->Draw("F,same");
	  Float_t x2[] = {18.3,18.3,18.7,18.7};
	  Float_t y2[] = {8.3,8.7,8.7,8.3};
	  TGraph *p2 = new TGraph(4,x2,y2);
	  p2->SetLineColor(0);
	  p2->SetFillColor(0);
	  p2->Draw("F,same");
	}
